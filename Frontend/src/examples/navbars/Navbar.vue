...
function toggleSidebar() {
  appStore.navbarMinimize();
}

function toggleConfigurator() {
  appStore.toggleConfigurator();  // Usamos la mutación de Pinia
}

// Computed
const isAbsolute = computed(() => appStore.isAbsolute);
const route = useRoute();  // Obtén la ruta actual directamente desde el router
const currentRouteName = computed(() => route.name);  // Obtén el nombre de la ruta

onMounted(() => {
  setMaterialInput(); // Esto debería activar la animación en el input de búsqueda
});
</script>
    ...
    <div class="px-3 py-1 container-fluid">
      ...
      >
        <div
          ...
          :label="{ text: 'Search here', class: 'form-label' }" />
